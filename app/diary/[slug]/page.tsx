  ...
  return (
    <div className="min-h-full bg-[#fffaf2] text-[#27211d]">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
        <Link href="/diary" className="text-sm font-medium text-[#a06c45] hover:underline">
          返回日记
        </Link>

        <article className="mt-8 overflow-hidden rounded-3xl border border-[#eadfce] bg-white/78 shadow-sm">
          {entry.cover_image ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={entry.cover_image} alt="" className="h-72 w-full object-cover" />
          ) : null}
          <header className="border-b border-[#eadfce] px-6 py-7 sm:px-8">
            <time className="text-xs font-semibold uppercase tracking-[0.18em] text-[#a06c45]">
