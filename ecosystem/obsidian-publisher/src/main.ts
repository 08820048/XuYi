...
export default class QmblogPublisher extends Plugin {
  ...
  async onload() {
    ...
    this.addRibbonIcon("upload-cloud", "发布到 XuYi", async () => {
  ...
  private setStatus(text: string, revertMs?: number) {
    ...
    if (revertMs) {
      setTimeout(() => {
        if (this.statusBarEl) this.statusBarEl.setText("XuYi");
      }, revertMs);
    }
  }
    ...
    const modal = new PublishModal(
      ...
      async (options, onProgress) => {
        ...
        try {
          ...
          if (result.success) {
            this.setStatus("XuYi \u2713", 3000);
          } else {
            this.setStatus("XuYi \u2717", 5000);
          }
          return result;
        } catch (e) {
          this.setStatus("XuYi \u2717", 5000);
