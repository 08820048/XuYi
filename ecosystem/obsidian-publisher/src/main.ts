...
export default class QmblogPublisher extends Plugin {
  ...
  async onload() {
    ...
    this.addRibbonIcon("upload-cloud", "发布到 XuYi'Blog", async () => {
    ...
    this.statusBarEl.setText("XuYi'Blog");
  ...
  private setStatus(text: string, revertMs?: number) {
    ...
    if (revertMs) {
      setTimeout(() => {
        if (this.statusBarEl) this.statusBarEl.setText("XuYi'Blog");
      }, revertMs);
    }
  }
  ...
  private openPublishModal(file: TFile, content: string, title: string) {
    const modal = new PublishModal(
      ...
      async (options, onProgress) => {
        this.setStatus("XuYi'Blog \u23F3");
        try {
          const result = await this.doPublish(file, content, options, onProgress);
          if (result.success) {
            this.setStatus("XuYi'Blog \u2713", 3000);
          } else {
            this.setStatus("XuYi'Blog \u2717", 5000);
          }
          return result;
        } catch (e) {
          this.setStatus("XuYi'Blog \u2717", 5000);
